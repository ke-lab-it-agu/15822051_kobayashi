...
"""
    ...
    return predicted_labels[:1] # 最大1つに制限
...
def load_simpleqs(path: str) -> List[Dict[str, Any]]:
    """SimpleQuestionsのTSV/タブ区切りファイルを読み込み、EL評価形式に変換する。"""
    print(f"Loading SimpleQuestions data from: {path}")
    dataset = []
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            for line_number, line in enumerate(f, start=1):
                try:
                    # タブで分割: [Subject QID] [Predicate Pxxx] [Object QID] [Question Text]
                    ...
                    # Subject QID (ELの正解として使用)
                    subject_qid = parts[0].strip()
                    # 質問文 (utterance)
                    question_text = parts[3].strip()

                    if subject_qid and question_text:
                        # SimpleQuestionsは通常、質問に一つの主要なエンティティ（Subject）しか含まない
                        target_qids = {subject_qid}
                        
                        dataset.append({
                            "uid": f"SQ_{line_number}",
...
def run_el_pipeline(input_path: str, output_path: str):
    ...
    dataset = load_simpleqs(input_path)
    if not dataset:
        return
    ...
    for idx, item in enumerate(dataset, start=1):
        ...
        for m in mentions:
            ...
            if target_qids:
                common_qids = target_qids & candidate_qids
...
INPUT_FILE_PATH = "../datasets/simpleqs/simpleqs_test.txt"
OUTPUT_FILE_PATH = "../my_result/simpleqs/sim_yes_rerank.json"
