...
# ファイルのパス
CORRECT_WIKIDATA_FILE_PATH = "../datasets/simpleqs/simpleqs_test.txt"
WIKIDATA_FILE_PATH = "../my_result/simpleqs/sim_choice_llama.json"
...
def calculate_metrics(correct_wikidata_ids, predicted_wikidata_ids):
    ...
    for i, true_wikidata_ids in enumerate(correct_wikidata_ids, 1):
        if i <= len(predicted_wikidata_ids):
            ...
            # TP, FP計
            true_positives = len(set(predicted_wikidata_ids_for_line) & {true_wikidata_ids})
            false_positives = len(set(predicted_wikidata_ids_for_line) - {true_wikidata_ids})
            ...
            recall = true_positives / 1 # simpleQuestionsのため分母は常に1
...
# correct_wikidata_idsとpredicted_wikidata_idsを読み込み
...
# print(correct_wikidata_ids)
# print(predicted_wikidata_ids)
