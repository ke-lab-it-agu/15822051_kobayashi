...
def get_p31_labels_for_entity(entity_qid: str) -> List[str]:
    """
    SQLite DBから、指定されたエンティティQIDが持つP31（instance of）のラベルを取得する。
    """
    p31_labels = []
    try:
        ...
        FROM 
            p31_map AS T1
        INNER JOIN 
            labels AS T2 ON T1.p31_qid = T2.qid
        WHERE 
            T1.entity_qid = ?;
        """
        cursor.execute(query, (entity_qid,))
        results = cursor.fetchall()
        conn.close()
        p31_labels = [row[0] for row in results]
    except sqlite3.OperationalError:
        print(f"    [DB_WARN] Could not connect to {DB_NAME} or tables are missing for QID {entity_qid}. (SQLite OperationalError)")
        p31_labels = ["DB_ERROR"]
    except Exception as e:
        print(f"    [DB_ERROR] An unexpected error occurred for QID {entity_qid}: {e}")
        p31_labels = ["DB_ERROR"]
    return p31_labels
...
# ハイパーパラメータ
N_LIST = 1000  # クラスタ数
N_PROBE = 50   # 検索時にチェックするクラスタ数 (速度と精度のトレードオフ)
M_PQ = 128          # PQ 分割数（最重要）
...
def extract_target_qids_from_sparql(sparql_query: str) -> Set[str]:
    """SPARQLクエリ文字列からすべてのWikidata QID (Qxxx) を抽出する。"""
    if not sparql_query:
        return set()
    ...
    # クエリ全体からQIDを検索し、セットとして返す
    qids = qid_pattern.findall(sparql_query)
    
    return set(qids)

def load_lcquad2(path: str) -> List[Dict[str, Any]]:
    """LC-QuAD2 JSON を読み込み、utterance とターゲットQIDを返す。"""
    print(f"Loading LC-QuAD2 data from: {path}")
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)

    dataset = []
    for item in data:
        # paraphrased_question を優先、その後 question
        utter = item.get("paraphrased_question")
        if not utter or utter == []:
            utter = item.get("question", "")
        
        # ターゲットQIDの取得: SPARQLクエリから抽出
        sparql = item.get("sparql_wikidata", "")
        target_qids = extract_target_qids_from_sparql(sparql)
...
def run_el_pipeline(input_path: str, output_path: str):
    ...
    try:
        dataset = load_lcquad2(input_path)
    ...
    for idx, item in enumerate(dataset, start=1):
        text = item["utterance"]
        uid=item["uid"]
        # LC-QuAD2から抽出したターゲットQID (Set型)
        target_qids = item["target_qids"] 
        
        # 質問レベルのヒットフラグを初期化
        question_hit_flag = False

        print("=" * 60)
        print(f"[{idx}/{total_questions}] UID={uid} Utterance: {text}")
        print(f" 🎯 Target QIDs for Utterance: {target_qids}")
...
INPUT_FILE_PATH = "../datasets/lcquad2/lcquad2_test.json"
OUTPUT_FILE_PATH = "../my_result/lcquad2/lcquad2_yes_rerank.json"
