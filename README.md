...
"wikidata_embeddings_06b_ali_classed.dat"というファイルが生成される。これがベクトルデータベースである。(作成に２０日ほど時間を要した)
...
webqsp,lcquad2,simplequestionsの３つのデータセットで実験ができる。それぞれのフォルダ内に存在しているvalue_fファイルはF1値を計算するためのファイルだ。使用したLLMはQwen2.5:7b、Gemma2:9b、Llama3.1:8bでエンベディングモデルはqwen3-embedding:0.6bである。それぞれ事前にインストールが必要である。コマンドは"ollama pull XXXX"である。XXXXに使用するモデルを入力。DBをパイプライン内で使用したのでsqliteも使えるようにしておく。sqlite3.37.2を使用した。
